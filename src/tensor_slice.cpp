...
TensorSlice::TensorSlice(std::vector<double>& data, const std::vector<std::size_t>& shape, std::size_t offset)
    : m_data(data), m_shape(shape), m_offset(offset) {}

TensorSlice TensorSlice::operator[](std::size_t index) {
    ...
    if(m_shape.size() == 1) {
        return TensorSlice(m_data, {}, m_offset + index);
    } else {
      std::size_t stride = 1; // number of elements in each sub-slice along this dimension.
      for(std::size_t i = 1; i < m_shape.size(); ++i) {
            stride *= m_shape[i];
      }
      return TensorSlice(m_data, std::vector<std::size_t>(m_shape.begin() + 1, m_shape.end()), m_offset + index * stride);
    }
}
